...
interface RiveSceneProps {
   ...
   * 상태 머신 이름.
   *
   * 시작→변신→마무리처럼 순서가 있는 연출은 반드시 이쪽을 쓴다 — 그 순서를 아는 건
   * 상태 머신뿐이라, 클립을 이름으로 직접 틀면 변신 과정이 통째로 빠지고 결과 포즈만 남는다.
   */
  stateMachine?: string;
  ...
  trigger?: string;
  /**
   * 상태 머신을 거치지 않고 곧바로 재생할 애니메이션 이름 (여러 개면 함께 재생된다).
   * 순서 없이 한 장면만 세워둘 때만 쓴다.
   */
