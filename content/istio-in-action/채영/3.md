...
> 데이터 플레인 = 엔보이(Envoy)들의 집합. 3장은 그 **엔보이 한 놈만 떼어내서** 클러스터 없이 도커로 직접 돌려보는 장이다.
> (2장은 istio 전체를 깔았다면, 3장은 그 안의 "근육 세포 하나"를 해부하는 느낌)

## 3.1 엔보이 프록시란 무엇인가?

- 엔보이 : 오픈소스의 고성능 **L7(애플리케이션 계층) 프록시**. 리프트(Lyft)가 만들어 CNCF에 기증.
- 리버스 프록시처럼 **백엔드 토폴로지를 클라이언트에게 숨기고**, 로드 밸런싱하고, 죽은 백엔드를 우회시킴.
- L3/L4 프록시(커넥션만 봄)와 달리 **HTTP 1.1 / HTTP2 / gRPC 등 애플리케이션 프로토콜을 이해** → 요청 수준 타임아웃·재시도·서킷 브레이커 같은 걸 할 수 있음.
- MongoDB, DynamoDB, AMQP 같은 프로토콜용 필터로 **확장 가능**.
- 애플리케이션 **밖(외부 프로세스)** 에서 동작 → 어떤 언어/프레임워크로 짰든 상관없이 이 기능들을 씀. (라이브러리 시대와 대비되는 핵심)
- 다재다능: **에지 프록시**(진입점) / **공유 프록시** / **서비스별 프록시(사이드카)** 어디든 쓸 수 있음. 이스티오는 서비스 인스턴스당 하나씩 배포하는 방식을 택함.

### 3.1.1 엔보이의 핵심 기능

엔보이를 이해하려면 이 3가지 개념을 알아야 함:
...
그 외 핵심 기능들:
- **서비스 디스커버리** : 전용 라이브러리(Eureka/Consul/주키퍼) 없이, 디스커버리 API에서 엔드포인트를 자동으로 찾음. (이스티오 컨트롤 플레인이 이 API를 구현)
- **로드 밸런싱** : 라운드로빈, 일관된 해싱(스티키) 등
- **헬스 체크** : 능동/수동으로 죽은 백엔드 골라내 우회
...
> 핵심 차이 = **동적 설정**. nginx는 설정 바꾸면 reload 하지만, 엔보이는 xDS로 실시간·무중단으로 바뀐다. (그래서 파드가 수시로 뜨고 죽는 쿠버네티스에 딱 맞음)

## 3.2 엔보이 설정하기

### 3.2.1 정적 구성 (static)

- YAML 파일에 리스너/루트/클러스터를 **직접 박아두는** 방식. 이번 실습이 이거.
...
> 2장에서 본 "istiod가 사이드카한테 설정을 뿌린다"가 바로 이 xDS. istiod = xDS 서버, 엔보이 = xDS 클라이언트.

## 3.3 엔보이 인 액션 (직접 실행)

> 클러스터 없이 **도커로 엔보이 한 놈만** 띄워서 해부해보자. httpbin(가짜 백엔드) + Envoy(프록시) + curl(클라이언트).
...
> **핵심:** 클라이언트는 요청 1번 보냈는데 엔보이가 뒤에서 **몰래 3번 더 재시도**(`upstream_rq_retry: 3`)했다. httpbin이 항상 500이라 결국 500을 받았지만, 만약 **간헐적 실패**였다면 사용자는 실패를 못 느끼고 성공 응답을 받았을 것. → 앱 코드 수정 0줄로 복원력을 얻는다.
